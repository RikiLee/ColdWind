	...
	void SwapChain::createSwapchain(VKContext& context, Window& window)
	{
		...
		if (m_swapChainExtent2D == vk::Extent2D()) {
			...
			if (surfaceFormats.result != vk::Result::eSuccess) {
				spdlog::error("Failed to get surface formats! Error code: {}", vk::to_string(surfaceFormats.result));
			...
			if (presentModes.result != vk::Result::eSuccess) {
				spdlog::error("Failed to get surface present modes! Error code: {}", vk::to_string(presentModes.result));
		...
		if (surfaceCapabilities.result != vk::Result::eSuccess) {
			spdlog::error("Failed to get surface capabilities! Error code: {}", vk::to_string(surfaceCapabilities.result));
		...
		if (returnValue.result == vk::Result::eSuccess) {
			...
			spdlog::error("Failed to create swapchain! Error code: {}", vk::to_string(returnValue.result));
			throw std::runtime_error("Failed to create swapchain!");
		}

		auto swapchainImages = device->getSwapchainImagesKHR(m_swapChain.get());
		if (swapchainImages.result != vk::Result::eSuccess) {
			spdlog::error("Failed to get swapchain images! Error code: {}", vk::to_string(swapchainImages.result));
			...
			for (size_t i = 0; i < swapchainImageSize; ++i) {
				...
				if (imageViewCreateResult.result != vk::Result::eSuccess) {
					spdlog::error("Failed to create image view! Error code: {}", vk::to_string(imageViewCreateResult.result));
